...
| Advanced Postgres telemetry | Yes | Query-counter deltas, active-session waits, plan evidence, lock attribution, compute/database telemetry | Explains a high-p99 interval with governed evidence in Delta and supports Insights/Genie workflows | Requires a project Observability configuration; the durable service-principal path currently has elevated Beta permission requirements |
