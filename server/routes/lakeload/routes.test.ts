...
describe('hard reset branch scope', () => {
  it('matches only LakeLoad demo, snapshot, and restore branches', () => {
    expect(isLakeLoadTestBranch({ name: 'projects/lakeload/branches/demo-live01' })).toBe(true);
    ...
    expect(isLakeLoadTestBranch({ name: 'projects/lakeload/branches/demo_' })).toBe(false);
  });
});
